...
class CommandMoneyPlacementNetworkClientTest {
    @Test
    fun shouldSendCommandMoneyPlacementDepositOverTcp() = runBlocking {
        sendAndCheck(
            operation = "MONEY_PLACEMENT_DEPOSIT",
            bills = 1000,
            coins = 0,
            reqNum = TestReqNum.value(30)
        )
    }

    @Test
    fun shouldSendCommandMoneyPlacementWithdrawalOverTcp() = runBlocking {
        sendAndCheck(
            operation = "MONEY_PLACEMENT_WITHDRAWAL",
            bills = 2500,
            coins = 50,
            reqNum = TestReqNum.value(31)
        )
    }

    private suspend fun sendAndCheck(operation: String, bills: Long, coins: Int, reqNum: Int) {
