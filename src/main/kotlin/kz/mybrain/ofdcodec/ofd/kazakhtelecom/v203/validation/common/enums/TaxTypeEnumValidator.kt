...
/**
 * Валидация TaxTypeEnum для протокола Казахтелеком v203.
 */
class TaxTypeEnumValidator {
    /**
    ...
    fun validate(container: JsonObject, key: String, path: String): List<ValidationError> {
        ...
        val allowed = Nomenclature.NomenclatureResponse.TaxTypeEnum.values().map { it.name }.toSet()
        if (value !in allowed) {
