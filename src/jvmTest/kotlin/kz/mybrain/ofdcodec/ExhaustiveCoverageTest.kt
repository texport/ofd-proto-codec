...
class ExhaustiveCoverageTest {
    ...
    fun testServiceValidatorsAndBuilders() {
        ...
        assertEquals("Адрес Кз", orgProto.address_kz)
        assertEquals("123456789012", orgProto.inn)
        assertEquals("12345", orgProto.okved)

        val kkmProto = KkmRegInfoBuilder().build(kkmReg)
        assertEquals("123", kkmProto.fns_kkm_id)
        assertEquals("456", kkmProto.serial_number)
        assertEquals("789", kkmProto.kkm_id)
        ...
        assertEquals(123, statsProto.geo_position?.latitude)
        assertEquals(456, statsProto.geo_position?.longitude)
        assertEquals("GPS", statsProto.geo_position?.source)
    ...
    fun testBuildersExhaustively() {
        ...
        assertEquals(2024, csProto.close_time.date.year)
        ...
        assertEquals(55, nomProto.nomenclature?.current_version)

        // Service Builder
        val serviceProto = ServiceRequestBuilder().build(nomJson)
        assertTrue(serviceProto.get_reg_info!!)
        assertEquals(12, serviceProto.security_stats?.geo_position?.latitude)
        ...
        assertEquals(OperationTypeEnum.OPERATION_SELL, ticketProto.ticket?.operation)
    }
    ...
    fun testHeaderCodec() {
        ...
        val decoded = decodedResult.header
